...
drw_text(Drw *drw, int x, int y, unsigned int w, unsigned int h, unsigned int lpad, const char *text, int invert, Bool markup)
{
    ...
    if (!render) {
        /* legacy behaviour kept from original */
    ...
    len = strlen(text);
    if (len) {
        /* compute extents for full text first */
        drw_font_getexts(drw->font, text, len, &ew, &eh, markup);
        th = eh;

        /* shorten text if necessary to fit width */
        size_t use_len = MIN(len, sizeof(buf) - 1);
        while (use_len && ew > w) {
            use_len--;
            drw_font_getexts(drw->font, text, use_len, &ew, &eh, markup);
            if (eh > th)
                th = eh;
        }

        if (use_len) {
            memcpy(buf, text, use_len);
            buf[use_len] = '\0';

            if (use_len < len) {
                /* add ellipsis (up to 3 dots) */
                for (i = (int)use_len; i && i > (int)use_len - 3; )
                    buf[--i] = '.';
            }
            ...
            if (render) {
                ty = y + (h - th) / 2;
                if (markup)
                    pango_layout_set_markup(drw->font->layout, buf, use_len);
                else
                    pango_layout_set_text(drw->font->layout, buf, use_len);
