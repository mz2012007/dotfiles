...
drawbar(Monitor *m)
{
	...
	if (m == selmon) {
		tw = statusw = m->ww - drawstatusbar(m, bh, 0, stext);
	}
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		/* hide unused and unselected tags */
		if (!(occ & (1 << i)) &&
	...
	w = TEXTW(m->ltsymbol);
	drw_setscheme(drw, scheme[SchemeNorm]);
	x = drw_text(drw, x, 0, w, bh, lrpad / 2,
	             m->ltsymbol, 0, False);
